...
async function ensureLocalEncryptionKey(
  repo,
  token,
  { forceCreateKey, forceDownloadKey }
) {
  if (forceCreateKey) {
    log.info('Forcing creation and upload of encryption key')
    return createAndStoreEncryptionKey(repo, token)
  }

  if (forceDownloadKey) {
    log.info('Forcing download of encryption key')
    return downloadAndStoreEncryptionKey(repo, token)
  }
...
async function downloadAndStoreEncryptionKey(repo, token) {
  const encryptionKey = await downloadEncryptionKey(repo, token)

  config.encryptionKey = encryptionKey
}

async function createAndStoreEncryptionKey(repo, token) {
