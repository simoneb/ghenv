...
async function ensureWorkflow(repo, token, { forceCreateWorkflow }) {
  ...
  const workflowContentString = fs
    ...
    .replace('${version}', ghenvPkg.version)
  ...
  log.debug(
    `Remote workflow version is ${existingVersion}, local is ${ghenvPkg.version}`
  )

  const outdated = semver.gt(ghenvPkg.version, existingVersion)
